...
class SnapbotGymClass():
    ...
    def step(self,a,max_time=np.inf):
        ...
        y_diff = p_torso_curr[1] - p_torso_prev[1]
        r_side = (y_diff / self.dt) * 1.0
        r += r_side

        r_y = p_torso_curr[1] * 0.1
        r += r_y
        ...
        # Lane keeping
        lane_deviation = p_torso_curr[0]
        r_lane = -np.abs(lane_deviation) * 0.5
        r += r_lane

        r_x_over = 0.0
        if abs(p_torso_curr[0]) > 0.50:
            r_x_over = -0.01
        r += r_x_over
        ...
        cos_theta = np.dot(heading_vec, y_vec) / np.linalg.norm(heading_vec)
        ...
        info = {'yaw_torso_deg_prev':yaw_torso_deg_prev,'yaw_torso_deg_curr':yaw_torso_deg_curr,
                ...
                'r_side': r_side, 'r_y': r_y, 'r_collision': r_collision, 'r_survive': r_survive, 
                'r_heading': r_heading, 'r_lane': r_lane, 'r_x_over': r_x_over, 'r_deg_over': r_deg_over}
