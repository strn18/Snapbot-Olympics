...
for epi_idx in epi_idx_list:
    ...
    pth_path = './task_2_sidewalk/weights/episode_%d.pth'%(epi_idx)
    ...
    for tick in range(max_epi_tick):
        ...
        reward_total += reward
        s = s_prime
        # target_r_total += info['r_x_over']
        # print(info['r_z'])
        if RENDER:
            if not gym.is_viewer_alive(): break
        max_y = max(max_y, gym.env.get_p_body('torso')[1])
    # print(f'target_r_total: {target_r_total}')
    if RENDER:
        gym.close_viewer()
    print(f'[{epi_idx}]: {max_y}')
    if max_y > max_record:
        max_idx = epi_idx
        max_record = max_y
