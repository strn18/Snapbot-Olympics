...
while True:
    ...
    for epi_idx in range(n_episode+1): # for each episode
        ...
        for tick in range(max_epi_tick): # for each tick in an episode
            ...
            p_torso_prev = gym.env.get_p_body('torso')
            _, _, geom1s_prev, _, _, _ = gym.env.get_contact_info()

            s_prime,reward,done,info = gym.step(a_np,max_time=max_epi_sec)

            p_torso_curr = gym.env.get_p_body('torso')
            _, _, geom1s_curr, _, _, _ = gym.env.get_contact_info()

            if 'floor' in geom1s_prev and 'floor' not in geom1s_curr:
                # print("JUMPED")
                jump_start_pos = p_torso_prev
            elif 'floor' not in geom1s_prev and 'floor' in geom1s_curr:
                if not initial_landed:
                    initial_landed = True
                else:
                    # print("LANDED!!!!!!!!")
                    # jump_distance = np.sqrt(sum((p_torso_curr[:2] - jump_start_pos[:2]) ** 2))
                    jump_distance = p_torso_curr[0] - jump_start_pos[0]
                    longest = max(longest, jump_distance)
        ...
        if (epi_idx%print_every)==0:
            ...
            print ("  reward:[%.1f] longest:[%.3f] epi_len:[%d/%d] buffer_size:[%d] alpha:[%.2f]"%
                (reward_total,longest,epi_tick,max_epi_tick,
        ...
        if (epi_idx%eval_every)==0:
            ...
            print ("  [Eval] reward:[%.3f] longest:[%.3f] epi_len:[%d/%d]"%
                (reward_total,longest,tick,max_epi_tick))
            
            max_longest = max(max_longest, longest)
            # if epi_idx == 650 and max_longest < 0.1:
            #     again = True
            #     break
