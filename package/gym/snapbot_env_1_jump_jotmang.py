...
class SnapbotGymClass():
    ...
    def __init__(self,env,HZ=50,history_total_sec=2.0,history_intv_sec=0.1,VERBOSE=True):
        ...
        self.initial_height    = self.env.get_p_body('torso')[2]
        ...
        print(self.env.rev_joint_names)
        print(self.env.ctrl_ranges)
        breakpoint()
    ...
    def step(self,a,max_time=np.inf):
        ...
        leg_list = ['leg_1_2', 'leg_2_2', 'leg_4_2', 'leg_5_2', 'leg_1_3', 'leg_2_3', 'leg_4_3', 'leg_5_3']
        ...
        qpos_prev          = dict()
        for leg in leg_list:
            qpos_prev[leg] = float(self.env.get_qpos_joint(leg))
        ...
        qpos_curr          = dict()
        for leg in leg_list:
            qpos_curr[leg] = float(self.env.get_qpos_joint(leg))
        ...
        # charge and jump
        cur_time = self.env.get_sim_time()
        # z_diff = p_torso_curr[2] - p_torso_prev[2]
        # r_height = 0
        charge_time = 0.5
        # if cur_time < charge_time:
        #     if z_diff < -1e-4:
        #         r_height = -z_diff * 10.0
        # else:
        #     if z_diff > 1e-4:
        #         r_height = z_diff * 100.0
        # r += r_height
        # if random.randint(0, 1000) == 42:
        #     print(f'r_height: {r_height}')
        r_height = 0
        if p_torso_curr[2] > self.max_height:
            r_height = p_torso_curr[2]
            self.max_height = p_torso_curr[2]
            print(r_height)
        r += r_height * 10.0

        # balancing
        R_torso = self.env.get_R_body('torso')
        roll, pitch, _ = r2rpy(R_torso)
        r_balance = - (abs(roll) + abs(pitch)) * 100.0
        # r += r_balance

        # leg balancing
        hold_legs = ['leg_1_2', 'leg_2_2', 'leg_4_2', 'leg_5_2']
        hold_penalty = sum(qpos_curr[leg] ** 2 for leg in hold_legs)
        # r -= hold_penalty
        # if random.randint(0, 1000) == 42:
        #     print(f'hold_penalty: {hold_penalty}')

        move_legs = ['leg_1_3', 'leg_2_3', 'leg_4_3', 'leg_5_3']
        if cur_time < charge_time:
            r_move = sum(qpos for qpos in qpos_curr.values()) * -10.0
        else:
            qpos_diff = [qpos_curr[leg] - qpos_prev[leg] for leg in move_legs]
            r_move = sum(qpos_diff) * 10.0
        # r += r_move
        # if random.randint(0, 1000) == 42:
        #     print(f'r_move: {r_move}')

        # breakpoint()
        # std1 = np.std(qpos_list1)
        # std2 = np.std(qpos_list2)
        # r_leg_balance = -1 * (std1 + std2) * 0.01
        # r += r_leg_balance
        # if random.randint(0, 1000) == 42:
        #     print(f'r_leg_balance: {r_leg_balance}')
