...
class SnapbotGymClass():
    ...
    def step(self,a,max_time=np.inf):
        ...
        _, _, geom1s_prev, _, _, _ = self.env.get_contact_info()
        ...
        _, _, geom1s_curr, _, _, _ = self.env.get_contact_info()
        ...
        r_jump_distance = 0.0
        if 'floor' in geom1s_prev and 'floor' not in geom1s_curr:
            self.jump_start_pos = p_torso_prev
        elif 'floor' not in geom1s_prev and 'floor' in geom1s_curr:
            if not self.initial_landed:
                self.initial_landed = True
            else:
                # jump_distance = np.sqrt(sum((p_torso_curr[:2] - jump_start_pos[:2]) ** 2))
                jump_distance = p_torso_curr[0] - self.jump_start_pos[0]
                r_jump_distance = jump_distance * 1.0
        r += r_jump_distance
        ...
        # r_forward = (x_diff / self.dt) * 0.01
        # r += r_forward

        # r_y_over = 0.0
        # if abs(p_torso_curr[1]) > 0.5:
        #     r_y_over = -0.1
        # r += r_y_over

        r_x = p_torso_curr[0] * 0.1
        r += r_x

        z_heading_vec = R_torso_curr[:, 2]
        z_vec = np.array([0, 0, 1])
        cos_theta = np.dot(z_heading_vec, z_vec) / np.linalg.norm(z_heading_vec)
        ...
        x_heading_vec = R_torso_curr[:, 0]
        x_vec = np.array([1, 0, 0])
        cos_theta = np.dot(x_heading_vec, x_vec) / np.linalg.norm(x_heading_vec)
        theta_rad = np.arccos(np.clip(cos_theta, -1.0, 1.0))
        theta_deg = np.degrees(theta_rad)
        r_x_deg_over = 0.0
        if theta_deg > 45.0:
        ...
        info = {'yaw_torso_deg_prev':yaw_torso_deg_prev,'yaw_torso_deg_curr':yaw_torso_deg_curr,
                ...
                'r_jump_distance': r_jump_distance, 'r_jump': r_jump,
